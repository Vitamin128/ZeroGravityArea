...
public:
    ...
    void addprovider(const BaseConnection::ptr &newconn, const Address &newhost,
    ...
    Discoverer::ptr AddDiscoverer(const BaseConnection::ptr &conn, const std::string &newmethod) {
        ...
        // std::unique_lock<std::mutex>lock(_mutex);
        {
            ...
            if (it == conns.end()) {
                ...
                LOG(NORMAL) << "创建Discoverer客户端" << std::endl;
            } else {
                newdiscoverer = it->second;
                LOG(NORMAL) << "服务发现成功" << std::endl;
            }
...
private:
    ...
    void notify(const std::string &method, const Address &host, const ServiceOptype &optype) {
        ...
        for (auto &item : it1) {
            LOG(NORMAL) << "发送服务通知 ["
                        << (optype == ServiceOptype::SERVICE_ONLINE ? "上线" : "下线")
                        << "] 方法: " << method << " 地址: " << host.first << ":" << host.second
                        << std::endl;
...
public:
    ...
    void onServiceResponse(const BaseConnection::ptr &conn, const ServiceRequest::ptr &svr_msg) {
        ...
        if (svopt == ServiceOptype::SERVICE_DISCOVERY) {
            LOG(NORMAL) << svr_msg->rid() << "服务发现:" << svr_msg->method() << std::endl;
            ...
            LOG(NORMAL) << svr_msg->host().first << ":" << svr_msg->host().second
                        << ",服务注册:" << svr_msg->method() << std::endl;
    ...
    void onShutdown(const BaseConnection::ptr &conn) {
        ...
        if (it.get() != nullptr) {
            ...
            LOG(NORMAL) << "服务提供者下线: " << it->host.first << ":" << it->host.second
                        << std::endl;
