...
public:
    ...
    void InitSearcher(std::string index_path) {
        ...
        if (!index->BuildIndex(index_path)) {
            std::cerr << "InitSearcher: BuildIndex failed for path: " << index_path << std::endl;
        }
    }
    ...
    void Search(const std::string &query, std::string *json_result) {
        ...
        // [修改前]：把查到的所有倒排元素无脑 append 到一起，导致不同关键词命中同一篇文档时，结果重复出现
        ...
        // 根据累加后的权重进行降序排序
        std::sort(
            inverted_list_all.begin(), inverted_list_all.end(),[](const InvertedElemCombined &e1, const InvertedElemCombined &e2) {
    ...
    std::string GetDesc(const std::string &content, const std::string &word) {
        auto iter = std::search(content.begin(), content.end(), word.begin(),
                                word.end(),[](char ch1, char ch2) {
                                    return std::tolower(ch1) == std::tolower(ch2);
                                });
        size_t pos = (iter != content.end()) ? std::distance(content.begin(), iter)
                                             : std::string::npos;
        ...
        if (pos == std::string::npos) {
            ...
            // 致命隐患：如果是中文字符（UTF-8占3字节），切断在字符中间，前端会出现“”乱码，甚至导致 JSON 解析崩溃。
            
            // [修改后]：UTF-8 边界安全探测。
            // 原理：UTF-8 多字节字符的“延续字节”总是以二进制 `10xxxxxx` 开头（即 0x80 ~ 0xBF 之间）。
