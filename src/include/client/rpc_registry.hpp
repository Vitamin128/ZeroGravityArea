...
public:
    ...
    // 向注册中心注册方法（携带自身地址）
    bool registrymethod(const BaseConnection::ptr &conn, const std::string &method,
                        const Address &host) {
        ...
        LOG(NORMAL) << "发送服务注册请求, method=" << method << ", host=" << host.first << ":"
                    << host.second << std::endl;
        BaseMessage::ptr rep;
        ...
        _host_loads[host] = load;
            ...
            Address GetHost() {
                ...
                for (const auto &h : _hosts) {
                    ...
                    std::cout << "[LB_DEBUG] 节点: " << h.first << ":" << h.second << ", 负载: " << l << std::endl;
                ...
                std::cout << "[LB_DEBUG] >>> 最终选中: " << best_host.first << ":" << best_host.second << std::endl;
                return best_host;
            }
...
public:
    ...
    bool ServiceDiscover(const BaseConnection::ptr &conn, const std::string &method,
                         Address &host) {
        {
            std::unique_lock<std::mutex> lock(_mutex);
            ...
            if (it != MethodHosts.end()) {
                if (it->second && !it->second->empty()) {
                    ...
                    LOG(NORMAL) << "命中本地服务缓存, method=" << method << ", host=" << host.first
                                << ":" << host.second << std::endl;
                    return true;
                }
            }
        ...
        }
        {
            ...
            LOG(NORMAL) << "服务发现完成, method=" << method << ", host=" << host.first << ":"
                        << host.second << std::endl;
            return true;
        }
    }
    ...
    void onServiceRequest(const BaseConnection::ptr &conn, const ServiceRequest::ptr &msg) {
        {
            ...
            if (msg->optype() == ServiceOptype::SERVICE_ONLINE) {
                ...
                if (it != MethodHosts.end()) {
                    ...
                    LOG(ERROR) << "收到下线通知但本地无该服务记录, method=" << msg->method()
                               << std::endl;
                    return;
                }
            } else {
